...
## Get data
get_weather_data() {
    weather=`curl "https://api.met.no/weatherapi/locationforecast/2.0/compact?lat=${LATITUDE}&lon=${LONGITUDE}"`
    # echo ${weather}

    if [ ! -z "$weather" ]; then
        weather_temp=`echo "$weather" | jq ".properties.timeseries.[0].data.instant.details.air_temperature"`
        weather_icon_code=`echo "$weather" | jq -r ".properties.timeseries.[0].data.next_1_hours.summary.symbol_code"`
        weather_max_temp=`echo "$weather" | jq "[.properties.timeseries[] | select(.time | startswith(\"2025-07-01\")) | .data.instant.details.air_temperature] | max"`
        weather_min_temp=`echo "$weather" | jq "[.properties.timeseries[] | select(.time | startswith(\"2025-07-01\")) | .data.instant.details.air_temperature] | min"`

        echo ${weather_temp}
        echo ${weather_icon_code}
        echo ${weather_max_temp}
        echo ${weather_min_temp}
